...
function enviarCorreoTareaEliminada($emailUsuario, $nombreUsuario, $datosTarea) {
    ...
    $para = $emailUsuario;
    $asunto = 'Tarea Eliminada - SAMICAM';
    $mensaje = '
<!DOCTYPE html>
<html lang="es">
<head>
...
<body style="font-family: -apple-system, BlinkMacSystemFont, Segoe UI, Roboto, Oxygen, Ubuntu, Cantarell, sans-serif; line-height: 1.5; color: #2d3748; background-color: #ffffff; margin: 0; padding: 20px; display: flex; justify-content: center; align-items: center; min-height: 100vh;">
    <div style="width: 100%; max-width: 480px; margin: 0 auto; background-color: white; border-radius: 8px; overflow: hidden; border: 1px solid #e2e8f0; box-shadow: 0 10px 20px rgba(0, 0, 0, 0.15);">
        <div style="background-color: #c53030; color: white; padding: 12px 24px; display: flex; align-items: center; justify-content: center; gap: 12px;">
            <h1 style="margin: 0; font-size: 18px; font-weight: 600; letter-spacing: 0.5px;">Tarea Eliminada</h1>
        </div>

        <div style="padding: 24px;">
            <p style="font-size: 16px; margin-bottom: 16px; color: #2d3748; display: flex; align-items: center; gap: 8px;">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z" stroke="#E53E3E" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            ...
            <div style="margin-bottom: 20px; color: #2d3748; font-size: 14px;">
                <p>Te informamos que la siguiente tarea ha sido <strong style="color: #c53030;">eliminada</strong> del sistema:</p>
            </div>

            <div style="background-color: #fed7d7; border-left: 3px solid #e53e3e; padding: 12px 16px; margin: 16px 0; font-size: 14px; border-radius: 0 4px 4px 0;">
                Esta tarea ya no está disponible en tu lista de tareas asignadas.
            </div>

            <div style="margin: 20px 0;">
                <div style="display: flex; margin-bottom: 12px; font-size: 14px;">
                    <span style="font-weight: 600; min-width: 100px; color: #c53030;">Tarea:</span>
                    <span style="flex: 1;">' . $datosTarea['titulo'] . '</span>
                </div>
                <div style="display: flex; margin-bottom: 12px; font-size: 14px;">
                    <span style="font-weight: 600; min-width: 100px; color: #c53030;">Dependencia:</span>
                    <span style="flex: 1;">' . $datosTarea['dependencia_nombre'] . '</span>
                </div>
                <div style="display: flex; margin-bottom: 12px; font-size: 14px;">
                    <span style="font-weight: 600; min-width: 100px; color: #c53030;">Fecha eliminación:</span>
                    <span style="flex: 1;">' . date('d/m/Y H:i') . '</span>
                </div>
            </div>

            <div style="background-color: #f7fafc; padding: 16px; border-radius: 4px; margin-top: 20px; font-size: 13px; color: #4a5568;">
                Si tienes dudas sobre esta eliminación, contacta a tu supervisor o al administrador del sistema.
            </div>
        </div>
...
';
    ...
    try {
        ...
        $mail->Subject = $asunto;
        $mail->Body = $mensaje;
        $mail->AltBody = 'Tarea eliminada en SAMICAM: ' . $datosTarea['titulo'];

        $mail->send();
        return true;
    } catch (Exception $e) {
        error_log('Error al enviar correo de tarea eliminada a ' . $para . ': ' . $mail->ErrorInfo);
        return false;
    }
}