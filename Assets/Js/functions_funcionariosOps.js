...
function fntMigrarInfo(idefuncionario) {
  // Primero obtenemos los datos del funcionario para mostrar su nombre
  let request = window.XMLHttpRequest
    ? new XMLHttpRequest()
    : new ActiveXObject("Microsoft.XMLHTTP");
  let ajaxUrl = base_url + "/funcionariosOps/getFuncionario/" + idefuncionario;
  ...
  request.onreadystatechange = function () {
    if (request.readyState == 4 && request.status == 200) {
      try {
        ...
        if (objData.status) {
          let nombreFuncionario = objData.data.nombre_completo;
          
          Swal.fire({
            title: "Migrar Funcionario",
            text: "¿Está seguro de migrar al Funcionario " + nombreFuncionario + " a Funcionarios de planta?",
            icon: "question",
          ...
          if (result.isConfirmed) {
            // Mostrar indicador de carga durante la migración
            Swal.fire({
              title: "Migrando funcionario...",
              text: "Por favor espere",
              allowOutsideClick: false,
              didOpen: () => {
                Swal.showLoading();
            ...
            let requestMigrar = window.XMLHttpRequest
              ? new XMLHttpRequest()
              : new ActiveXObject("Microsoft.XMLHTTP");
            let ajaxUrl = base_url + "/funcionariosPlanta/migrarFuncionario";
            let strData = "ideFuncionarioOps=" + idefuncionario;
            
            requestMigrar.open("POST", ajaxUrl, true);
            requestMigrar.setRequestHeader(
              "Content-type",
              "application/x-www-form-urlencoded"
            );
            
            requestMigrar.onreadystatechange = function () {
              if (requestMigrar.readyState == 4) {
                if (requestMigrar.status == 200) {
                  try {
                    let objData = JSON.parse(requestMigrar.responseText);
                    if (objData.status) {
                      Swal.fire("Migrado!", objData.msg, "success");
                      tableFuncionarios.api().ajax.reload();
                    } else {
                    ...
                    console.error("Error al procesar la respuesta:", error);
                    console.log("Respuesta recibida:", requestMigrar.responseText);
                    Swal.fire("Error", "Ocurrió un error al procesar la respuesta", "error");
                  }
                } else {
                  console.error("Error HTTP:", requestMigrar.status);
                  Swal.fire("Error", "Error en la comunicación con el servidor: " + requestMigrar.status, "error");
                }
              }
            };
            
            requestMigrar.send(strData);
          }
        });
      } else {
        Swal.fire("Error", "No se pudo obtener la información del funcionario", "error");
      }
    } catch (error) {
      console.error("Error al procesar la respuesta:", error);
      Swal.close();
      Swal.fire("Error", "Ocurrió un error al procesar la información del funcionario", "error");
    }
  }
};
}
